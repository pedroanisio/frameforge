...
- **`[Adopted]`** **Site ⇄ source.** [tooling/gen_docs.py](./tooling/gen_docs.py) generates
  the site's content pages — schema reference from the schema, fixture gallery from the
  renderer, spec/grammar/changelog verbatim, and the SDK guide/API (`sdk.md`/`sdk-api.md`,
  committed generated snapshots). The **only hand-written page in the `mkdocs.yml` nav is
  `docs/index.md`**. Other hand-written pages live under `docs/` but are **out of nav** —
  design records (`architecture.md`, `adr-0001-*.md`, `roadmap-draft.md`) and realized
  proposals (`framegraph-*-proposal.md` / `*-improvements.md`); these are neither in the
  site nor build artifacts. Everything else under `docs/` is a `make docs` build artifact.
