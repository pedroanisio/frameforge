...
function RectObj({ doc, o }) {
  ...
  return <div data-frameforge-object={o.id || ""} data-frameforge-type="rect" style={st} />;
}
...
function TextObj({ doc, o, active }) {
  ...
  const content = o.spans
    ? o.spans.map((sp, i) => {
        ...
        return (
          <span key={i} data-frameforge-span={sp.id || i} style={{
  ...
  return (
    <div data-frameforge-object={o.id || ""} data-frameforge-type="text" style={wrapStyle}>
      {useFit ? (
...
function GroupObj({ doc, o, cw, ch, active }) {
  ...
  return (
    <div data-frameforge-object={o.id || ""} data-frameforge-type="group" data-layout-kind={layout?.kind || "free"} style={st}>
...
function ChipRowObj({ doc, o }) {
  ...
  return (
    <div data-frameforge-object={o.id || ""} data-frameforge-type="chip_row" style={{
...
function UmlMarkerGlyphObj({ doc, o }) {
  ...
  return (
    <svg data-frameforge-object={o.id || ""} data-frameforge-type="uml.marker_glyph"
...
function UmlBoxObj({ doc, o }) {
  ...
  return (
    <div data-frameforge-object={o.id || ""} data-frameforge-type={o.type} style={{
      position: "absolute", left: x, top: y, width: w, height: h,
...
function ComponentObj({ doc, o, active }) {
  ...
  return (
    <div data-frameforge-object={o.id || ""} data-frameforge-type="component" data-frameforge-component={o.component || ""} style={baseStyle}>
...
function UmlGlyphBoxObj({ doc, o }) {
  ...
  return (
    <svg data-frameforge-object={o.id || ""} data-frameforge-type={o.type}
...
function ContainerObj({ doc, o, cw, ch, active }) {
  ...
  return (
    <div data-frameforge-object={o.id || ""} data-frameforge-type="container" style={{
...
function LegendObj({ doc, o, cw, ch, active }) {
  return (
    <div data-frameforge-object={o.id || ""} data-frameforge-type="legend" style={{
      position: "absolute", left: 0, top: 0, width: cw, height: ch,
      opacity: o.opacity != null ? o.opacity : 1,
    }}>
      {(o.items || []).map((item, i) => {
...
function TableView({ doc, o, absolute = true }) {
  ...
  return (
    <div data-frameforge-table={o.id || ""} style={outer}>
...
function FlowBlock({ doc, block, width }) {
  ...
  if (type === "math") return <div data-frameforge-type="math" style={{ margin: "10px 0", textAlign: "center", fontFamily: "serif", fontSize: 16 }}><MathDisplay tex={block.tex || block.text} mathml={block.mathml} /></div>;
...
function FlowPageCanvas({ doc, page, active = true }) {
  ...
  return (
    <div
      data-frameforge-page={active ? "active" : "thumb"}
      data-page-mode="flow"
...
function ReadingOrder({ page, objectIndex }) {
  ...
  return (
    <ol data-frameforge-reading-order={page.id || ""} style={{
...
function App() {
  ...
  useEffect(() => {
    window.__FRAMEFORGE_VIEWER__ = {
      loadDoc(nextDoc) {
        const normalized = normalizeFrameForgeDoc(nextDoc);
  ...
  const openFile = (e) => {
    ...
    r.onload = () => {
      try {
        ...
        const normalized = normalizeFrameForgeDoc(parsed);
        ...
        setErr("Couldn't parse that FrameForge document: " + e2.message);
      }
    };
    r.readAsText(f);
    e.target.value = "";
