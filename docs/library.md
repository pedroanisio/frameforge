...
`frameforge.library` is the content library absorbed from the predecessor
...
`src/frameforge/library/data/` — translated once from the v0.1 sources
...
`frameforge.sdk.expand`. The cover/agenda/hex packs reference pack-scoped
text styles the consumer must supply — merge `support_text_styles(...)`
into `defs.tokens.text_styles` next to a theme:

```python
from frameforge.library import load_symbols, load_theme, support_text_styles
...
```python
from frameforge.library import (honeycomb_capability_map, load_example,
                                module_hub_radial)
