...
```powershell
docker pull ghcr.io/pedroanisio/frameforge:2.6.1
```
...
cd frameforge
docker build -t frameforge:2.6.1 .
```
...
```powershell
docker build -t frameforge:2.6.1 --build-arg INSTALL_GOOGLE_FONTS=0 .
```
...
**If you build, set the plugin's Runtime image to `frameforge:2.6.1`** in Step 7
— the default points at the registry copy you do not have.

### Verify (either path)

```powershell
docker image inspect frameforge:2.6.1 --format "{{.Id}}"
```

Use `ghcr.io/pedroanisio/frameforge:2.6.1` as the name instead if you pulled.
...
```powershell
docker run --rm frameforge:2.6.1 version
```

Use `ghcr.io/pedroanisio/frameforge:2.6.1` throughout this step instead if you
pulled in 4A.

✅ **Pass**: prints the package version, the model `HEAD_VERSION`, and a build
stamp. The version should read `2.6.1`.
...
docker run --rm -v "${PWD}:/workspace:ro" frameforge:2.6.1 bash -c "ls /workspace"
```
...
docker run --rm `
  ...
  frameforge:2.6.1 bash -c "ls /library && touch /publish/.probe && echo MOUNTS OK"
```
...
|---|---|---|
| Runtime image | `ghcr.io/pedroanisio/frameforge:2.6.1` | **`frameforge:2.6.1` if you built it in 4B** — the default is a registry copy you do not have |
...
| Container starts but the plugin cannot find the image | 7 | You built locally but left the Runtime image at the `ghcr.io/...` default. Set it to `frameforge:2.6.1`. |
| `ls /workspace` shows nothing | 5b | Drive not shared → Docker Desktop → Settings → Resources → File sharing |
| Version prints something older than `2.6.1` | 5a | Stale local image: `docker pull` again |
...
| Fonts fall back to a generic sans | — | That family is not in the image. List what is: `docker run --rm frameforge:2.6.1 fonts` |
