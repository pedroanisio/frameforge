...
| `inert-stroke-declaration` | A stroke-painted shape (`line`, `polyline`, `polygon`, `path`, `curve`, `bezier`, `connector`) declares its stroke as `style: {color, width, dash}` — the shape of the pre-P3 bundle. Those keys **validate**: `Style.color` is *text* colour, `Style.width` is *box* width, `Style.dash` is unrelated to `stroke_dasharray`. None is stroke paint, so the authored appearance is discarded and the shape renders `#000`/1px (`line`/`connector`, which have an engine fallback) or **invisible** (the rest, which do not). | Use the P3 single form: paint in `stroke`, geometry in `stroke_style` — e.g. `stroke: '#d5d0c6'` + `stroke_style: {stroke_width: 1}`. `tooling/codemod.py --fix-inert-stroke` rewrites a whole document. The finding message carries the exact replacement. |
...
| `containment` | An object's resolved page-space box extends outside the canvas beyond a 1-unit tolerance. The audit descends into group-local children after row/column/grid/wrap arrangement; `decorative` does not exempt geometry. | Move/resize the object, or set `containment: allowed` for intentional bleed. On a group, that consent applies to its subtree. `PageBuilder.bleed()` stamps both containment and accessibility intent. |
...
| `text-truncated` | *(default; disable only with `--no-text-fit` / `text_fit=False`)* A text object materially lost content to the containment net — lines dropped, or glyphs cut beyond rounding tolerance. `SILENT` means the clip was the default, not an authored `overflow`/`text_overflow: ellipsis`/`max_lines`. | Enlarge the box, shorten the text, or opt in explicitly so the clip is acknowledged; the finding names the object, page, and the head of the dropped text (issue #44). For positioned text, size with `fit_width()` / MCP `fit_text`. |
| `layout-overflow` | *(default; disable only with `--no-text-fit` / `text_fit=False`)* A typed layout-overflow signal beyond the truncation set: an `overflow: visible` text object spilling its box, or a flow-mode line the Knuth–Plass engine had to emit wider than its column. Contained clips are reported as `text-truncated`, not here. | Widen the box/column, break the offending token, or accept the spill knowingly. In `diagnostics.overflow` / `sdk.overflow_report()`, `needed` is the post-layout extent and `unwrapped_width` is the pre-wrap width needed to avoid breaking. |
| `tabular-box-model` | ≥6 top-level absolutely-positioned text objects form an at least 85%-complete regular grid — probably a hand-built table. Objects tagged `meta.role: annotation`, `furniture`, or `lettering` are excluded. | Author true tabular data as a `row`/`column`/`grid` group or `table` object (§3.3); tag genuine non-tabular furniture with its semantic role. |
| `collision` | *(default; disable only with `--no-check-collision`)* Two top-level text objects on the **same layer** overlap in drawn **ink** without both declaring `overlap: allowed` — an *unintended* text-on-text overlap (collision-gate/2026-07). Render-time and ink-based: a static box check floods, so this runs the layout and compares glyph extents. ERROR above `GROSS_COLLISION_AREA` (2000 px², minor axis > 8 px), WARN below, so measurement noise never fails a build. The finding names the metrics mode (`estimate` is unverified per PALS's Law; `--real-metrics` makes it reproducible where the faces are installed). Cross-layer overlap is exempt (legitimate z-order); a consented overlap is an effect, not a collision. | If the overlap is intentional (watermark, caption-over-image), set `overlap: allowed` on **both** objects; otherwise separate their boxes. The render diagnostics `collisions` channel / `sdk.collision_report()` carry the full typed record: `ids, page, layer, area, overlap, metrics, boxes, texts`. **`ids` is `[None, None]` unless both objects were authored with an `id`** — generated documents rarely carry them, so `boxes` (both ink rectangles) and `texts` (a bounded excerpt of each) are what locate the pair. |
