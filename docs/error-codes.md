...
| `layout-overflow` | *(opt-in via `--text-fit`)* A typed layout-overflow signal beyond the truncation set: an `overflow: visible` text object spilling its box, or a flow-mode line the Knuth–Plass engine had to emit wider than its column (an unbreakable word or token). Contained clips are reported as `text-truncated`, not here. | Widen the box/column, break the offending token, or accept the spill knowingly; the full typed record (box, needed extent, policy, detail) is in the render diagnostics `overflow` channel / `sdk.overflow_report()`. |
