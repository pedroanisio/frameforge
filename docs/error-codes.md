...
| `collision` | *(opt-in via `--check-collision`)* Two top-level text objects on the **same layer** overlap in drawn **ink** without both declaring `overlap: allowed` — an *unintended* text-on-text overlap (collision-gate/2026-07). Render-time and ink-based: a static box check floods, so this runs the layout and compares glyph extents. The finding names the metrics mode (`estimate` is unverified per PALS's Law; `--real-metrics` makes it reproducible where the faces are installed). Cross-layer overlap is exempt (legitimate z-order); a consented overlap is an effect, not a collision. | If the overlap is intentional (watermark, caption-over-image), set `overlap: allowed` on **both** objects; otherwise separate their boxes. The render diagnostics `collisions` channel / `sdk.collision_report()` carry the full typed record (ids, page, layer, area, overlap, metrics). |
