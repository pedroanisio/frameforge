---
title: "Improving FrameForge for Reference Image Recreation"
status: >-
...
context: "Written after recreating an abstract mixed-media reference image through frameforge.sdk over the FrameForge MCP renderer."
disclaimer:
  notice: >-
...
# Improving FrameForge for Reference Image Recreation
...
|---|---|---|
| 1 | Page-space PNG crop/zoom | **Partial** — crop/zoom landed in the *measurement* lane (`measure_image` crops, `compare_images` regions, `vectorize_image` crop, `workspace` viewport) but the MCP *render* tools (`run_sdk_code` / `run_sdk_client` / `render_frameforge_yaml`) still render whole pages only. |
| 2 | Crop around an object id | **Open** — no render tool accepts an object id. |
| 3 | Combined inspect report (PNG + structural facts) | **Open** — no `inspect` in `src/frameforge/sdk` or `src/frameforge/mcp`. |
...
| 6 | Stroke ergonomics (`cap`/`join` aliases) | **Delivered** — `frameforge.sdk.paint.stroke(width, color=…, cap=…, join=…)` normalises to the P3 paint/geometry split. |
...
reference image closely with FrameForge. The observations come from a concrete
...
texture into a FrameForge vector document, then rendering it through MCP.
...
The crop must use FrameForge page coordinates, not pixel coordinates. That keeps
...
directly into FrameForge objects.

### 5. Texture and dry-brush macros

The SDK needs procedural texture helpers that lower to ordinary FrameForge
...
mixing. FrameForge should make blend modes and masks easy to author.
...
render_frameforge_yaml(..., raster_png=True, crop=[...], scale=4)
inspect_frameforge_yaml(..., crop=[...], scale=4)
...
5. Add reference-image analysis helpers for palette, regions, line skeletons,
   and texture maps.
6. Add an approximate vectorization pipeline that emits editable FrameForge
   primitives.
...
To recreate images closer to the original, FrameForge should become better at
three things:
