...
disclaimer:
  ...
  date: "2026-07-03"
status: analysis
scope: what `libfy.md` means for FrameForge, grounded in the live tree
---
...
`libfy` is **not a feature request — it is the name for a meta-loop FrameForge
...
## 1. FrameForge is the textbook subject for libfy — twice over

libfy's claim: *constructive work → curated library of (parts + know-how) →
capability compounds.* FrameForge is a construction tool, so it is a libfy
...
language — `src/frameforge/library/__init__.py`: *"the content library absorbed
...
aspirational for FrameForge. It is an unusually precise description of what is
**already committed**. Every rung is populated:

| Rung (concrete → abstract) | Live artifact | Consume API |
|---|---|---|
| **Assets** | font packs `.fp` (`src/frameforge/fontpack.py`), corpus media, `FigureAsset` (imported PDF/EPUB) | `fg-font --install`, `scope_font_pack`, `place_imported_figure` |
| **Parts** | symbol packs via `use`, `components`, widgets, `Group` | `load_symbols`, `use`/`component` → `src/frameforge/sdk/expand.py` |
...
| **Rules / tokens** | 7 house-style theme token packs (`src/frameforge/library/themes.py`) | `load_theme` → merged into `defs.tokens` |
...
| **Know-how / checklists** | `skills/typeface-and-colour/` (12 measurable gates), `src/frameforge/coach/` rubrics, ADRs, the MCP `describe_capabilities` catalog, validator codes R1–R12 | Skill invocation, `get_guide`, `describe_capabilities` |
| **The oracle** | `tests/fixtures/b1/` — the frozen reference the whole thing is diffed against | golden `oracle.lock.json` |

The finding is the strong one: **libfy describes FrameForge's existing
...
turn easier to libfy."* In most systems that is a slogan. In FrameForge it is a
verifiable property:
...
- `use`/`component` instances are stamped `meta.source_symbol` /
  `meta.source_component` at lowering time (`src/frameforge/sdk/expand.py`). The
...
FrameForge is **consume-complete and extract-incomplete.**
...
hand-ported them into `src/frameforge/library/generators.py`; someone will
...
FrameForge:** a `tooling/` command (and matching MCP tool) that takes one finished
...
- **The value is the curation gate, not the volume.** The definition stresses
  *curated, annotated* twice. The failure mode is 85 cookbook examples and a
  ...
  FrameForge already has the defense (validator, oracle lock, `gen_status`
...
libfy is the name for FrameForge's extract→curate→compound loop; every rung of its
