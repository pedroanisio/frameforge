...
2. **The `frameforge:2.8.0` image present locally.** Verify:
...
> `frameforge:2.8.0` argument to match. `ghcr.io/pedroanisio/frameforge:2.8.0`
...
| Fonts fall back to a generic sans | The family is not in the image. `docker run --rm frameforge:2.8.0 fonts` lists what is available. |
