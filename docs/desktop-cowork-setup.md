...
2. **The `frameforge:2.6.1` image present locally.** Verify:
...
> `frameforge:2.6.1` argument to match. `ghcr.io/pedroanisio/frameforge:2.6.1`
...
| Fonts fall back to a generic sans | The family is not in the image. `docker run --rm frameforge:2.6.1 fonts` lists what is available. |
