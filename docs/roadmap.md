...
| Authoring-session findings (2026-07-27) | [`frameforge-issue-drafts.md`](../frameforge-issue-drafts.md) + [migration record](migration-2.7-authoring-feedback.md) | ✅ Deployed to `main` in `84a5ddb`: shared text metrics/`fit_width`, deep explicit containment, default text-fit validation, whitespace preservation, dash normalization, sparse-grid roles/density, and unwrapped overflow width. Post-deployment backlog review found no matching open issue; related [#44](https://github.com/pedroanisio/frameforge/issues/44) was already closed as completed. |
...
| 2 | Accessibility / tagged export | gap | ✅ **SVG done**, ✅ **HTML ahead of SVG**, PDF/UA open | `svg.py` a11y_wrap (decorative/role/alt/actual_text), root lang/title/desc, `tooling/check_accessibility.py`; tests. **2026-07-02:** reading-order *DOM ordering* was retired — DOM order IS paint order, and reordering emission painted listed content beneath unlisted backgrounds; `reading_order` now rides as `data-reading-order` metadata on the page group (paint order stays layer/z/document). **2026-07-31 (HTML port):** *derived* semantics landed as a domain service (`domain/services/a11y.py`) and are consumed by the HTML backend — a `group` announces as a group, bare connector geometry is hidden instead of announced as an unnamed graphic, a word-glyph icon gets a real name while a raw symbol is hidden rather than mislabelled, and `ImageObject.label` finally reaches AT (the model spells it `label`, not `alt`, so no authored-field path had ever read it). **SVG deliberately still emits authored-only semantics**: consuming the derived ones shifts every golden byte, so opting SVG in is an explicit decision with an oracle re-pin attached — one call site, whenever it is taken. PDF/UA awaits a PDF backend and must consume the metadata, not DOM order. |
...
| **B3** | True 3D scene graph (nodes / instancing / hierarchy) | T2 direction | Ch8–11, ¶43–45 | L→XL | B1✓, B2✓ | **UNBLOCKED — `docs/adr-0005-3d-scene-graph.md`** (2026-07-05, operator: Option B **dual model** — a document-carried `scene3d` object that renders *lowered* to deterministic 2D **or** *fully-3D* natively). Prereqs delivered; staged 5a–5e (5a model → 5b lowered → 5c determinism → 5d hierarchy/instancing → 5e fully-3D backend). First build: 5a |
...
| **B9** | Curvature & arc-length API (curves/surfaces) | T2 high | Mortenson §6.7 | S–M | — | **DELIVERED** (2026-07-04): `CubicBezier.{derivative,curvature,arc_length}` + `polyline_length`; `test_geometry_curvature.py` + `surface_curvature` (K/H via the fundamental forms; `test_geometry_surface_curvature.py`). Residual: none |
