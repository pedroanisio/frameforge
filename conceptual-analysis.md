...
> **RESOLVED (2026-06-24).** The orchestrator (`Renderer`) and `normalize_doc` now live under `framegraph/rendering/application/`; `tooling/render_fixtures.py` re-exports them for its CLI. `framegraph/sdk/conform.py` and `framegraph/rendering/infrastructure/latex/document.py` import them from the package, so **`framegraph/` no longer imports up into `tooling/`** — the inverted dependency is gone, and `tests/test_package_boundary.py` keeps it from regressing. Package-readiness blocker #4 is cleared (codebase-standards §16).
