---
title: "FrameForge SDK — Consolidated Proposal (refined)"
status: superseded — realized in frameforge.sdk (generated API reference: docs/sdk-api.md); retained as design rationale
supersedes:
  - frameforge-sdk-interface.md
  - frameforge-sdk-geometry.md
derived_from:
  - frameforge-v2.ebnf
  - frameforge-v2-style.ebnf
  - anthropic_claude_deck_improved_frameforge.yml (measured fixture)
  - frameforge-geom.js (executed reference implementation)
references:
...
disclaimer: >
  ...
  specification. The `@frameforge/*` package names describe a language-neutral
  interface and are not shipped JavaScript packages. The Python implementation
  now lives under `frameforge.sdk` and is tested in `tests/test_sdk.py`. The
...
# FrameForge SDK — Consolidated Proposal (refined)
...
regression tests. FrameForge needs the same, and the reference implementation
proved why (§8).
...
```
@frameforge/model      Tier 0  GENERATED from JSON Schema. Closed types, value types,
                               discriminated unions. The only generated package.
@frameforge/geometry   Tier 0  Pure math: Vec2/Vec3, Mat3/Mat4, Path, parametric
                               sampling, projection. No document dependency; unit-testable
                               against the formulas in §3.3.
@frameforge/validate   Tier 1  validateStaticRules + the rule catalogue (§4). Catches
                               everything the schema cannot express.
@frameforge/author     Tier 2  DocumentBuilder, handles, builders.
   ├─ /macros                  theme(), md``, intermediate methods (defer-vs-flatten).
   └─ /draw                    Path/Frame/Scene3D builders → VisualObjects.
@frameforge/expand     Tier 3  Asset resolution, hashing, use/component instantiation,
                               and post-expansion validation (§6).
@frameforge/io         —       serialize (closed write) / parse (forgiving read).
@frameforge/conform    —       Golden corpus + math property tests (§8).
```
...
|---|---|---|
| `@frameforge/model` | `frameforge.sdk.model` | imports `models.frameforge` as the sole authority; exposes `Document`, `HEAD_VERSION`, validation, and plain-dict conversion |
| `@frameforge/geometry` | `frameforge.sdk.geometry` | `Vec2`, `Vec3`, `Mat3`, `Mat4`, `CubicBezier`, `Path`, and the quarter-circle kappa helper |
| `@frameforge/validate` | `frameforge.sdk.validate` | `ValidationReport`, `Issue`, `validate_static_rules()` over Pydantic structure, the existing static validator, SDK reference checks, target checks, and path parsing |
| `@frameforge/author` | `frameforge.sdk.author` | `DocumentBuilder`, `PageBuilder`, nominal `Handle` values for defs/tokens, symbol/component authoring helpers, and runtime handle-kind checks |
| `@frameforge/author/macros` | `frameforge.sdk.macros` | `theme()`, `md()`, and `paragraph()` macros that lower to token definitions and `Inline` / flow fragments |
| `@frameforge/author/draw` | `frameforge.sdk.draw` | `Frame` data-space mapping and `Scene3D` mesh/surface/extrude/revolve helpers that emit 2D visual objects |
| `@frameforge/expand` | `frameforge.sdk.expand` | deterministic asset/font hash pinning, `use` lowering, simple `component` lowering, and post-expansion model validation |
| `@frameforge/io` | `frameforge.sdk.io` | forgiving JSON/YAML parse for newer documents plus canonical JSON/YAML serialization |
| `@frameforge/conform` | `frameforge.sdk.conform` | proxy-render SVG extraction and per-page SHA-256 golden helpers |

The Python binding deliberately does not generate or redeclare core model types.
Every public document-producing API validates through `models.frameforge.Document`.
...
### 3.3 Geometry algebra (`@frameforge/geometry`)
...
## 4. Tier 1 — the consolidated rule catalogue (`@frameforge/validate`)
...
## 5. Tier 2 — authoring (`@frameforge/author`)
...
## 6. Tier 3 — expansion and reproducibility (`@frameforge/expand`)
...
## 7. io — serialization (`@frameforge/io`)
...
## 8. Conformance and testing (`@frameforge/conform`) — promoted to first-class
...
**Math property tests for `@frameforge/geometry`.** Because the geometry package
...
**Build sequence.** (1) `@frameforge/model` codegen — nothing else is real until
the generated types exist. (2) `@frameforge/validate` with the §4 catalogue.
(3) `@frameforge/conform` seed corpus (the deck), pinned. (4) `@frameforge/author`
+ `/macros`, then `/draw` on top of `@frameforge/geometry`. (5) `@frameforge/expand`
...
- Merged the **two package layouts** into one map (§2) and integrated `@frameforge/geometry` and `/draw`.
- Recorded the **implemented Python mapping** in §2.1: `frameforge.sdk.{model,geometry,validate,author,expand,io,conform}`.
