#!/usr/bin/env python3
"""
build_schema.py — generate framegraph-v2.schema.json FROM the Pydantic models.

This closes complement recommendation #3: the schema is no longer hand-authored
and lagging — it is emitted from `models/framegraph.py` (the single source of
...
Usage:
    python3 build_schema.py                 # write ../schema/framegraph-v2.schema.json
...
SCHEMA_PATH = os.path.normpath(os.path.join(HERE, "..", "schema", "framegraph-v2.schema.json"))
...
def main(argv=None):
    ...
    if args.check:
        ...
        if on_disk != text:
            print("STALE: schema/framegraph-v2.schema.json differs from a fresh build. Run build_schema.py.")
            return 1
