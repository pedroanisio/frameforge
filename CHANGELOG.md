...
**Version:** `2.6.1` · **Status:** PROPOSED / partially-implemented · **Date:** 2026-07-23
...
## 2.6.1 — fix: the runtime, the gates, and two renderer contracts get their pins (2026-07-23)

PATCH: no schema, `$defs`, or field-contract change — a document authored against
`2.6.0` validates and renders identically. Every entry below is a defect closed
against the 2.6.0 tree, plus two regression pins that were written test-first and
had never been committed alongside the fix they guard.
...
- **Authored `\n` under `white_space: pre-*`** — page-mode text layout collapsed
  ...
  model documents. Pinned by `tests/test_pre_line_breaks.py` (hard break per mode,
  wrapping still applies inside each segment, `shrink_to_fit` re-layout preserves the
  breaks, `normal` unchanged).
- **Multi-run spans no longer rely on chunk anchoring** — a centred or right-aligned
  ...
  the middle anchor for byte-stability. Pinned by
...
- **The bump moves nine → eleven sites, and the sweep can no longer go silent** —
  ...
  through `.mcp.json`. Nothing moved it and nothing pinned it, so every bump since the
  plugin landed would have left the launcher pulling the previous release against a
  current manifest. It is now a `bump_version.py` site, pinned by
