#!/usr/bin/env python3
"""FrameGraph — the logo, as a standalone, reusable FrameGraph document.

This file is the **canonical home of the FrameGraph mark**. ``mark()`` below is the
...
ready to drop onto any surface. They are written to the repo's canonical brand-asset
location, ``brand/``.

Run from the repository root::

    uv run python examples/framegraph_logo.py   # writes brand/framegraph-*.svg + brand/framegraph-logo.fg.yaml
"""
...
# --------------------------------------------------------------------------- #
# THE MARK — single source of truth. Imported by examples/framegraph_seed_deck.py.
# --------------------------------------------------------------------------- #
def mark(page, cx, cy, size, *, frame=INK, graph=FRAME, node_fill=CANVAS):
    """The FrameGraph mark — *the framed derivation*. The name's two halves fuse
...
MASTERS = [
    ("logo-primary",  "framegraph-mark.svg",          True),
    ("logo-mono",     "framegraph-mark-mono.svg",     True),
    ("logo-reversed", "framegraph-mark-reversed.svg", False),   # keeps its ink ground
    ("logo-favicon",  "framegraph-mark-favicon.svg",  True),
    ("logo-lockup",   "framegraph-wordmark.svg",      True),    # the horizontal lockup
]
...
def _transparent(svg: str) -> str:
    if _INJECTED_WHITE_BG not in svg:
        raise RuntimeError(
            ...
            "_INJECTED_WHITE_BG in examples/framegraph_logo.py")
    return svg.replace(_INJECTED_WHITE_BG, "", 1)


def build() -> DocumentBuilder:
    b = DocumentBuilder(title="FrameGraph — Logo", profile="deck", lang="en")
