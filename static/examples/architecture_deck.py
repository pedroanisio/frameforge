"""FrameGraph v2 — AS-IS Architecture & Evolution Paths.

A publication-grade architecture deck, authored in the FrameGraph SDK and
rendered through the FrameGraph MCP. Every architectural claim is grounded in
...
"""
import os, math
from framegraph.sdk import DocumentBuilder, PageBuilder, Mat3, serialize
...
b=DocumentBuilder(title="FrameGraph v2 — AS-IS Architecture & Evolution Paths",
...
def slide(kicker,title="",*,dark=False,sub=None):
    ...
    T(MX,H-32,600,"FrameGraph v2  ·  AS-IS Architecture",size=CAP,color=FAINT,weight=600,spacing=0.4)
...
def s_title():
    ...
    R([0,0,W,H],fill=BAND)
    R([0,0,W,6],fill=ACC)
    # framed-derivation glyph (the FrameGraph brackets) top-left
    gx,gy=MX,150
    for dx in (0,208):
    ...
    # title block
    T(MX,382,1200,"FrameGraph v2",size=HERO,weight=800,color="#FFFFFF",lh=1.0)
    ...
    T(MX,H-52,1200,"Grounded in the live repository — not memory.   Generated by Claude Opus 4.8 via FrameGraph MCP · 2026-07-03.",
      size=CAP,color=FAINT_ON_BAND,mono=True)

def s_exec():
    slide("Executive summary","What FrameGraph is — and is not")
    y=body_top()
...
def s_pipeline():
    ...
    T(x3+18,y2+66,W-MX-x3-40,"One FrameGraph document in → one RenderedArtifact out. The right seam for backends "
...
def s_mcp():
    ...
    y=body_top()
    groups=[("Author → render",ACC,["run_sdk_code","run_sdk_client","render_framegraph_yaml","write/read/list_sdk_clients"]),
...
def s_export():
    ...
    y=body_top()
    # targets table
    T(MX,y,700,"Render targets  (framegraph-render --to)",size=SML,weight=800,color=ACC_INK,spacing=0.5)
    ...
    for i,(t,d,c) in enumerate([("svg_to_objects","existing SVG → FrameGraph objects (round-trips through render)",ACC),
                                ("propose_from_image","raster → UNVERIFIED FrameGraph draft",SIGNAL),
...
def s_part2():
    ...
    T(MX+22,y+18,660,"FrameGraph  —  declarative render IR",size=LEAD,weight=800,color="#FFFFFF")
...
def s_compare():
    slide("Capability comparison","FrameGraph vs SVG.js vs Snap.svg")
    y=body_top()-6
    dims=[("Shape creation API","have","have","have","full primitive set both sides"),
          ...
          ("Raster→vector reconstruction","have","na","na","FrameGraph-only (vision MCP)"),
          ("Multi-target / print fidelity","have","na","na","FrameGraph-only (PDF/TeX/HTML)")]
    cols=[("Dimension",470),("FrameGraph",150),("SVG.js",130),("Snap.svg",130),("Note",0)]
    rw=W-2*MX
...
def s_risks():
    ...
    risks=[("Verifiability ↔ interactivity tension","An animation/event runtime can erode the deterministic snapshot the gates depend on. Keep interaction in a consuming layer."),
           ...
           ("Scope creep toward a DOM toolkit","Chasing SVG.js feature-for-feature would trade FrameGraph's differentiators (verification, multi-target, vision) for parity that browsers already own.")]
    ...
    steps=[("1","Ship P0 scene-query over the IR — small, pure-Python, unlocks everything downstream."),
           ...
           ("4","Publish the FrameGraph-vs-toolkit comparison as living docs so parity claims stay honest."),
...
def s_appendix():
    ...
    cols=[("Inspected (file : what it grounds)",[
              "docs/models/framegraph.py — model, 1,804 LOC, HEAD 2.4.1",
              "…Document/Page/Layer/ObjBase/Style/Paint/Layout/Defs",
              "src/framegraph/sdk/*.py — 33 modules (author…planar)",
              ...
              "src/framegraph/mcp/ — 24 tool defs",
    ...
    T(MX,y+484,W-2*MX,"Disclaimer: no statement here should be taken for granted — claims are grounded in the cited source at the stated version (HEAD 2.4.1); anything not verifiable in-tree is labelled uncertain.  Generated by Claude Opus 4.8 via the FrameGraph MCP · 2026-07-03.",
