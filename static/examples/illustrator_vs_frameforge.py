#!/usr/bin/env python3
"""CAPABILITY TEARDOWN — Adobe Illustrator ⇄ FrameForge v2.

A granular, feature-by-feature teardown: every Illustrator tool, panel, and
capability the source book documents, set against FrameForge v2's coverage, with
...
Provenance / epistemic contract (CLAUDE.md rules 1 & 2):
  ...
  * FrameForge coverage quotes docs/capability-manifest.json (generated from the
    live tree, gated by tests/test_capability_manifest.py) and the SDK surface.
  * Verdicts are unbiased. Where FrameForge has no answer (an explicit non-goal),
    the glyph is a hollow ○, not spin.

Build (from the frameforge root, src-layout):
    PYTHONPATH=src:docs python static/examples/illustrator_vs_frameforge.py [--out FILE]
"""
...
from frameforge.sdk import DocumentBuilder, serialize, stroke  # noqa: E402
...
# Cool technical palette. Illustrator = its own brand amber; FrameForge = cyan.
COLORS = {
    ...
    "fg":      "#0f7d88",   # FrameForge teal-cyan (4.5:1)
...
# "reframed" = the capability exists in BOTH tools, but FrameForge reaches it by
...
def col_headers(p, y):
    ...
    p.text([FV_X, y, FV_W, 10], "FRAMEFORGE COVERAGE", style="colhd")
...
GROUPS = [
    ("A · SELECT & EDIT PATHS", "sel-edit", [
        ...
        ("AI-04", "Compound paths / Pathfinder",
         ...
         "has", "planar.union / intersect / subtract / divide — booleans "
         "emitted as even-odd paths (holes native)."),
        ("AI-05", "Shape Builder",
         "Drag across shapes to merge / subtract them interactively.", "24·523",
         "invert", "REFRAMED: the drag-merge gesture is planar.union / "
         "subtract by declaration."),
        ("AI-06", "Scissors & Knife",
         "Cut a path at a point, or slice across objects.", "25·365",
         "has", "planar.split_at(t) (arc-length scissors) + cut_along "
         "(knife via half-plane booleans)."),
        ("AI-47", "Offset path",
         "Object > Path > Offset expands a path outward / inward.", "26·3298",
         "has", "planar.offset_polygon (closed, miter, collapse-aware) + "
         "stroke_outline for open paths."),
    ...
    ("C · COLOUR SYSTEM", "colour", [
        ...
        ("AI-16", "Recolor Artwork",
         ...
         "has", "recolor() — one-call palette remap: tokens, paint literals, "
         "gradient stops; plus gradient_map."),
        ("AI-17", "Live Paint",
         "Fill regions bounded by overlapping paths interactively.", "24·319",
         "has", "planar.fill_regions — every bounded region of an overlay "
         "as its own fillable face (+ the region toolkit)."),
        ("AI-18", "Colour science",
         "Colour groups & harmony via the Color Guide panel.", "24·684",
         "has", "chevreul.color_guide() — the six harmonies for any base "
         "colour + WCAG contrast tools."),
        ("AI-19", "Patterns",
    ...
    ("E · GRADIENT · EFFECT · STYLE", "appearance", [
        ...
        ("AI-30", "Live effects",
         ...
         "has", "ordered `effects` stack (2.4.0): kinds repeat, preset + "
         "param entries, applied first→last."),
        ...
        ("AI-32", "Appearance stack",
         ...
         "has", "`appearance` stack (2.4.0): the geometry painted once per "
         "pass — fill / stroke / opacity, bottom→top."),
    ]),
...
META = {
    ...
    "AI-04": {"conf": "H", "dim": "F", "why": "Pathfinder booleans landed (W1): planar.union/intersect/subtract/divide on flattened rings, Greiner-Hormann with deterministic degeneracy handling."},
    "AI-05": {"conf": "H", "dim": "W", "why": "The interactive drag becomes a declared boolean (W1) — same merge outcome, tool call instead of gesture."},
    "AI-06": {"conf": "H", "dim": "F", "why": "Path surgery landed (W1): split_at by arc length, cut_along as half-plane booleans."},
    ...
    "AI-16": {"conf": "H", "dim": "W", "why": "recolor() landed (W4): one call remaps tokens by name/value, paint literals and gradient stops, case-insensitively."},
    "AI-17": {"conf": "H", "dim": "F", "why": "fill_regions landed (W1): boolean-atom decomposition of a small overlay — every bounded region its own fillable face (authoring scope, ≤8 shapes)."},
    ...
    "AI-18": {"conf": "H", "dim": "W", "why": "color_guide() landed (W4): the six Chevreul harmonies for any base colour, ready to feed closed_palette/recolor."},
    ...
    "AI-30": {"conf": "H", "dim": "F", "why": "The ordered effects stack landed (W4, 2.4.0 additive): repeatable kinds, presets with param overrides, declarative and non-destructive by construction."},
    "AI-32": {"conf": "H", "dim": "F", "why": "The appearance stack landed (W4, 2.4.0 additive): multiple fill/stroke passes over one geometry, bottom→top."},
    ...
    "AI-47": {"conf": "H", "dim": "F", "why": "Offset landed (W1/W2): offset_polygon closed with miter + collapse detection; stroke_outline covers open paths."},
...
def manifest_identity():
    """Reproducibility identity for the FrameForge side: sha256 + version + count."""
    import hashlib
    ...
    return {
        ...
        "frameforge_version": data.get("version", "?"),
...
def source_identity():
    ...
    return {
        ...
        "builder": "static/examples/illustrator_vs_frameforge.py",
    }
...
def audit_rows():
    ...
    FrameForge manifest capability names its coverage prose references (IFG-004).
    """
    import re
    ...
    for grp_title, grp_key, rows in GROUPS:
        for fid, feat, ai, ordn, cov, fg in rows:
            ...
            out.append({
                ...
                "frameforge_coverage": fg,
                "frameforge_manifest_refs": refs,
...
def build():
    b = DocumentBuilder(
        title="Capability Teardown — Illustrator vs FrameForge",
        profile="report",
        lang="en",
    )
    ...
    b.meta(
        provenance=(
            ...
            "GraphQL). FrameForge coverage quotes docs/capability-manifest.json."
        ),
    ...
    p.text([M, 84, CW, 12], "ADOBE ILLUSTRATOR 2024 · 2025 · BMG106   vs   FRAMEFORGE v2.3.0",
           style="nmeta")
    ...
    p.text([M, y, 452, 116],
           ...
           "against FrameForge's capability manifest. Evidence cites SRC·ORDINAL. This is a "
           "feature-surface comparison, not a maturity benchmark: FrameForge is a PROPOSED "
    ...
    demo = [("gid", "AI-nn", "monospace feature id (chapter-ordered)"),
            ...
            ("covlbl", "● HAS", "FrameForge verdict glyph + label"),
            ("fgev", "evidence", "the FrameForge capability that backs the verdict")]
    ...
    p.text([M + 44, ry + 40, CW - 88, 30],
           "The capability exists in BOTH tools — but FrameForge reaches it by "
    ...
    p.text([M + 44, ry + 84, CW - 88, 12],
           ...
           "FrameForge NAMES it by id.", style="folio", color="sub", align="start")
    p.text([M + 44, ry + 100, CW - 88, 12],
           "AI-39  Illustrator TRACES a raster from a menu   →   "
           "FrameForge calls the vectorize_image tool.", style="folio",
    ...
    src = source_identity()
    idlines = [("FrameForge", f"v{ident['frameforge_version']} · {ident['manifest_capabilities']} caps"),
               ...
               ("Full audit", "…_vs_frameforge.audit.json"),
    ...
    # -- EXAMPLES: Illustrator action → FrameForge source ---------------- #
    ...
    p.text([M, y, 452, 56],
           ...
           "Illustrator gesture on the left and as the FrameForge source the SDK emits "
    ...
    for tag, cov, title, ai_do, fg_code in ex:
        ...
        # right: FrameForge source
        rx = M + 18 + half
        p.text([rx, ey + 58, half, 12], "FRAMEFORGE SDK", style="colhd", color="fg")
        for j, ln in enumerate(fg_code.split("\n")):
            p.text([rx, ey + 74 + j * 13, half, 12], ln, style="cite", color="ink",
                   align="start")
        ey += 130

    # -- REVERSE GAPS: what FrameForge has that Illustrator doesn't ------- #
    p = page(b, "reverse", tab="THE OTHER DIRECTION")
    y = head(p, 74, "// the comparison runs both ways",
             "What FrameForge has that Illustrator lacks")
    p.text([M, y, 452, 96],
    ...
    p.text([M, y, 452, 148],
           f"Across {total} Illustrator capabilities from three manuals, FrameForge has a "
           ...
           f"generative-AI features exist in both — but FrameForge reaches them by "
    ...
    p.text([M + 26, ty + 26, CW - 52, 34], "Illustrator is a hand.",
           style="h2", color="bg")
    p.text([M + 26, ty + 60, CW - 52, 34], "FrameForge is a grammar.",
           style="h2", color="fg")
    ...
    p.text([M + 20, hy + 36, CW - 40, 60],
           "FrameForge v2 is a PROPOSED format: no renderer is conformant, the SVG "
    ...
    p.text([M, py + 26, CW, 90],
           ...
           "FrameForge coverage quotes docs/capability-manifest.json (278 capabilities, "
    ...
    p.text([M, py + 120, CW, 12],
           ...
           + f" · manifest {_mid['manifest_sha256'][:12]} (v{_mid['frameforge_version']})"
...
def main() -> int:
    ...
    ap.add_argument("--out", default=os.path.join(os.path.dirname(__file__) or ".",
                                                  "illustrator_vs_frameforge.fg.yaml"))
    args = ap.parse_args()
    doc = build()
    ...
    audit = {
        "artifact": "illustrator_vs_frameforge capability teardown",
        ...
        "version": {
            ...
            "frameforge_manifest": manifest_identity(),
            "external_corpus": corpus_identity(),
        },
        "frameforge": manifest_identity(),
        "sources": SOURCES,
        "rubric": {
    ...
    print(f"wrote {audit_path}  ({len(audit['features'])} rows · "
          ...
          f"manifest {audit['frameforge']['manifest_sha256'][:12]} · "
