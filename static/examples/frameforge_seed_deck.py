#!/usr/bin/env python3
"""FrameForge — Seed Pitch, authored *in* FrameForge (the canonical seed).

This is the deck that defines FrameForge as "the output layer for the agent era,"
authored through the FrameForge SDK and rendered by the project's own renderer —
...
Run from the repository root::
    ...
    uv run python examples/frameforge_seed_deck.py     # writes docs/seed/*.svg + the .fg.yaml
"""
...
from frameforge_sdk import DocumentBuilder, radial_gradient, rgba, serialize
...
# source of truth — examples/frameforge_logo.py), imported here rather than redefined.
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))   # the examples/ dir
from frameforge_logo import fan, mark, wordmark  # noqa: E402,F401
...
def on_profile(page):
    """Top-right state chip: FRAMEFORGE ✓ ON-PROFILE (the gate, promoted to chrome)."""
    rx = W - M
    page.text([rx - 230, 30, 110, 12], "FRAMEFORGE",
...
# document, examples/frameforge_logo.py, so the logo can never diverge between the
...
def s01_cover(b):
    ...
    page.text([M, 470, 600, 110],
              "AI can read the world. FrameForge lets it produce the finished "
...
def s02_shift(b):
    ...
    page.text([M, 580, W - 2 * M, 24], "FrameForge is building that layer.",
...
def s05_solution(b):
    ...
    headline(page, 112, "FrameForge turns intent into a finished, checked file.",
             size=44, w=980)
    lead(page, 252, "You — or your AI — describe what you want, or point it at "
                    "your data. FrameForge assembles the artifact, checks it the "
...
def s06_how(b):
    ...
    card(page, cols[1], "Step 2", "Build",
         "FrameForge lays it out correctly — spacing, alignment, brand, "
...
def s09_reframe(b):
    ...
    lead(page, 318, "Design tools, slide software, document systems, dashboards, "
                    ...
                    "artifact. FrameForge is the layer underneath all of them.",
         w=1090, size=16.5)
...
def s15_vision(b):
    ...
    lead(page, 472, "FrameForge is the layer that makes that output real, "
...
def s16_logo(b):
    """Endcap — the FrameForge mark, shown as a system, not an illustration: one
...
def build() -> DocumentBuilder:
    b = DocumentBuilder(title="FrameForge — Seed Pitch (canonical)",
...
def main() -> int:
    ...
    for idx, svg in enumerate(render_page_svgs(doc), start=1):
        with open(os.path.join(out_dir, f"seed-{idx:02d}.svg"), "w", encoding="utf-8") as fh:
            fh.write(svg)
    with open(os.path.join(ROOT, "docs", "frameforge-seed.fg.yaml"), "w", encoding="utf-8") as fh:
        fh.write(serialize(doc, format="yaml"))
    print(f"Wrote {len(doc.pages)} SVG(s) to {out_dir} + docs/frameforge-seed.fg.yaml")
