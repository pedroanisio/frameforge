...
FrameForge v2 is a document/graphics DSL for decks, diagrams, books, letters,
...
The **Pydantic model at `docs/models/frameforge.py` is the single source of
...
```
docs/models/frameforge.py     SOURCE OF TRUTH (Pydantic v2)
...
src/frameforge/               the Python package (strictly downstream of the models)
  ...
  ├── vision/       inverse lane: pixels → UNVERIFIED FrameForge drafts
  ...
  ├── cli.py        `frameforge-render` front door (multi-backend)
...
## 4. The SDK (`src/frameforge/sdk/`, 32 modules, ~12k LoC)
...
## 5. Rendering (`src/frameforge/rendering/`, ~12.2k LoC)
...
## 6. Vision (`src/frameforge/vision/`, ~6.3k LoC)

The inverse renderer: proposes FrameForge objects **from pixels**. Its cardinal
...
- **coach/** (~1.4k LoC) — the Vector Construction Coach (POC). It does **not**
  ...
  analysis. Imports only `frameforge.sdk` + stdlib (OpenCV lazily in `ingest`).
...
## 8. MCP server (`src/frameforge/mcp/`, ~6.3k LoC)
...
`python -m frameforge.mcp`. The tool wrappers live in `server.py` (~1.6k lines);
...
- **Sessions / meta:** `get_guide`, `describe_capabilities`, `list_fonts`,
  ...
  (`frameforge://session/{id}/…`) and a guide prompt.
...
- **`frameforge-render`** (`cli.py`) — one front door dispatching a document to a
...
and converters/ingest (`pdf_to_frameforge_yml`, `vectorize_image`, `codemod`),
...
broad: decks (`architecture_deck`, `naval_engineering_deck`, `frameforge_seed_deck`),
...
- **Local checkout hygiene.** This working tree carries an untracked top-level
  `frameforge/` directory (stale build artifacts / an old package copy); the real,
  tracked package is `src/frameforge/`. It is harmless but can mislead casual
  inspection — `conftest.py` explicitly manages the `frameforge`-module shadow so
  imports resolve to `docs/models/frameforge.py`.
...
`src/frameforge/**`, `tooling/`, `tests/`, and `static/examples/`. Figures
