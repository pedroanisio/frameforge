...
# F1 build study — three.js (WebGL) vs FrameGraph SDK
...
**FrameGraph SDK** (a deterministic vector document). This note compares the two
...
- `static/examples/f1_spec_sheet.py` + `static/examples/f1_car_3d.py` — the
  FrameGraph model + dossier authored in this repo.
...
**FrameGraph SDK — technical dossier** (338 components · 1,219 layers · 110 effects · 9 SDK subsystems)

![FrameGraph F1 dossier](assets/f1_framegraph_dossier.png)
...
was **vendored locally**. The FrameGraph piece has no runtime dependency: it is
...
| Dimension | three.js (`f1laminar`) | FrameGraph SDK (this repo) |
|---|---|---|
...
- a body **lofted from elliptical cross-section slices** — three.js instances a
  `CylinderGeometry` slice along X ("laminated construction"); FrameGraph sweeps
...
tonemap, soft shadows — each visually load-bearing. FrameGraph's effect entries
...
- **FrameGraph is flatter / diagram-like**, but the car is more *legibly* an F1
...
three.js is a real-time 3D **engine**; FrameGraph is a document/graphics **DSL**
...
### If the goal were to close the "premium" gap in FrameGraph
