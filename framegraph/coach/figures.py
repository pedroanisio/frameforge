...
Where :mod:`framegraph.coach.clean` lifts *any* traced lines (image-agnostic),
...
analyzes comes from :func:`framegraph.coach.ingest.ingest` upstream.
"""
...
# ═══════════════════════════════════════════════════════════════
#  Figure model (analyze) + bridges to/from FrameGraph objects
# ═══════════════════════════════════════════════════════════════
@dataclass
class FigureFrame:
...
def dominant_contour(objs: Sequence[Obj]) -> list:
    ...
    The realistic input is :func:`framegraph.coach.ingest.ingest`'s region
...
def analyze(
    ...
    ``source`` is either a list of FrameGraph objects (the dominant contour is
...
def to_polygon_obj(
    ...
    """Wrap a point list as a FrameGraph ``polygon`` object."""
    obj: Obj = {
        "type": "polygon",
