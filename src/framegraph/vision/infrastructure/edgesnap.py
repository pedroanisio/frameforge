...
:mod:`framegraph.vision.domain.geometry` fits lines and intersects them (exact
...
lazily so ``import framegraph.vision`` stays cheap (mirrors ``matchscore.py``).
"""
...
def edge_crossings_along(image_bytes: bytes, a: Point, b: Point, *,
    ...
    crossings — feed it to :func:`~framegraph.vision.domain.geometry.fit_line`.
    """
    import numpy as np
