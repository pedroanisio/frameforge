"""Clip-path constructors for the FrameGraph SDK.

These assemble the plain ``{shape, args}`` dicts the authoritative ``ClipPath``
model already accepts (``models.framegraph.ClipPath``) and the SVG proxy already
...
Pair them with :meth:`framegraph.sdk.PageBuilder.group` / :meth:`~PageBuilder.frame`
...
from framegraph.sdk.geometry import Path as _GeomPath, Vec2
...
def normalize_clip(clip: Any) -> dict[str, Any] | str:
    ...
    (→ :func:`clip_polygon`), a :class:`~framegraph.sdk.Path` (→ :func:`clip_path`),
