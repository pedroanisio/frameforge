...
- :func:`_new_generated_yaml` — content-hash diffing of the FrameGraph YAML a
...
- :func:`describe_capabilities` — LIVE introspection of the authoritative
  document model (``models/framegraph.py``, loaded through the same
  ``framegraph.sdk.model`` mechanism the pipeline uses), so an agent can look
...
def _pinned_session_fonts(
    ...
    from framegraph.mcp.sessions import _session_id
