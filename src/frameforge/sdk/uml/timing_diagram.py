...
from frameforge.sdk.uml._composer_base import ComposedDiagram
...
def compose_timing_diagram(
    ...
    *,
    ...
    """
    if options is None:
        defaults = TimingDiagramOptions()
        available_width = max(defaults.label_width + 1.0, canvas_size[0] - 2 * defaults.margin)
        opts = TimingDiagramOptions(lane_width=min(defaults.lane_width, available_width))
    else:
        opts = options
