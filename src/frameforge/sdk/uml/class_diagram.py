...
laid-out FrameForge `visual` block ready for the renderer. Uses the
pure-Python Sugiyama layout from `frameforge.layout` for hierarchical
...
from frameforge.sdk.sugiyama import LayoutResult, SugiyamaConfig, sugiyama_layout
from frameforge.sdk.uml._v2 import normalize_visual, visual_to_document, visual_to_page
...
class ComposedDiagram:
    ...
    Attributes:
        visual: The fully-laid-out `visual` block. Insert into a
            FrameForge document under the `visual` key.
    ...
    def __post_init__(self) -> None:
        """Normalize every emitted object at the v2 package boundary."""
        self.visual = normalize_visual(self.visual)

    def to_visual(self) -> dict[str, Any]:
        """Return the normalized visual block for low-level composition."""
        return self.visual
    ...
    def to_page(
        ...
        *,
        ...
        """Return a complete v2 absolute page containing the diagram."""
        return visual_to_page(self.visual, page_id=page_id, canvas_size=canvas_size)
    ...
    def to_document(
        ...
        *,
        ...
        """Return a complete, model-valid FrameForge v2 document."""
        return visual_to_document(
            self.visual,
            title=title,
            page_id=page_id,
...
def _format_attribute_for_width(attr: dict[str, Any]) -> str:
    ...
    Mirrors `frameforge.renderers.uml._format_attribute` exactly. Kept
...
def _format_operation_for_width(op: dict[str, Any]) -> str:
    """Mirror of `frameforge.renderers.uml._format_operation` for width estimation."""
    visibility_prefix = {"public": "+", "private": "-", "protected": "#", "package": "~"}
...
def compose_class_diagram(
    ...
    *,
    ...
    Returns:
        A `ComposedDiagram` whose `.visual` field is a FrameForge
        `visual` block dict (tokens, layers, objects), ready to drop
        into a FrameForge document.
