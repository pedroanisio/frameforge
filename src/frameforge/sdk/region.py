"""Region operations over a flat FrameForge object list.

These are the SDK primitives behind *region-based transformation*: treat an
ingested drawing (e.g. the output of
:func:`frameforge.vision.infrastructure.svg_import.svg_to_objects`) as a list of
...
from frameforge.sdk.clip import clip_rect
...
def place_region(
    ...
    **fields: Any,
    ...
    optional ``transform`` (a :class:`~frameforge.sdk.Mat3`) is applied about the
...
def extract_objects(
    ...
    *,
    ...
    ``source`` is anything :func:`frameforge.sdk.place_figure` accepts — a
    :class:`~frameforge.sdk.DocumentBuilder`, a document/dict, a
    ``.fg.yaml``/``.fg.json`` path, or a :class:`~frameforge.sdk.figure.FigureRef`.
    ...
    """
    from frameforge.sdk.figure import FigureRef, load_figure   # lazy: avoid import cycle
