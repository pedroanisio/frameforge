"""Document-level Markdown → a FrameForge flow document.

The SDK already lowers *inline* Markdown (:func:`frameforge.sdk.macros.md` —
...
- ```` ```frameforge ```` pattern directives degrade to a structured
  warning until the pattern bridge (#29) lands — never a silent drop.

The returned document has been through :func:`~frameforge.sdk.model
.validate_document` — converter output is untrusted until the schema says
otherwise (PALS's Law)::

    from frameforge.sdk import from_markdown
...
def from_markdown(
    ...
    *,
    ...
    construct that degrades — today the ```` ```frameforge ```` directive.
    ...
    while i < len(lines):
        ...
        if fence:
            ...
            if language == "frameforge":
                sink.append(
                    "```frameforge pattern directive skipped — pattern-composed "
