"""Live FrameForge figure import helpers."""
...
from frameforge.sdk.geometry import Mat3
from frameforge.sdk.io import parse
from frameforge.sdk.model import HEAD_VERSION, to_plain_dict
...
class FigureAsset:
    ...
    ``FigureRef`` is for live FrameForge page imports. ``FigureAsset`` is for the
...
class FigureRef:
    ...
    imports the selected page's objects as normal FrameForge children, preserving
    ...
    def from_document(cls, document: Any, *, page: str | int | None = None) -> "FigureRef":
        """Create a figure reference from a FrameForge document object or dict."""
    ...
    def from_path(
        ...
        *,
        ...
        """Create a figure reference from a YAML or JSON FrameForge file."""
    ...
    **fields: Any,
    ...
    if caption_box is not None and caption_text is not None:
        ...
        caption: dict[str, Any] = {
            ...
            "text": caption_text,
            "style": style,
            "meta": {"frameforge.figure.part": "caption"},
        }
        if id_prefix is not None or figure.id is not None:
...
def _as_document_dict(value: Any) -> dict[str, Any]:
    ...
    if isinstance(data, dict) and "layers" in data:
        return {"dsl": "FrameForge", "version": HEAD_VERSION, "pages": [data]}
    raise ValueError("figure source must be a FrameForge document or page")
