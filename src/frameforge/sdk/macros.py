"""Authoring macros that lower to ordinary FrameForge model fragments."""
...
from frameforge.sdk.author import DocumentBuilder, Handle
from frameforge.sdk.geometry import Path
from frameforge.sdk.paint import hatch, stroke
...
def md(text: str) -> list[Any]:
    ...
    ``$math$`` and ``[label](href)`` links, plus a FrameForge
...
def span(
    ...
    **style: Any,
    ...
    spans and plain strings for :meth:`~frameforge.sdk.PageBuilder.text` and
    :meth:`~frameforge.sdk.FlowBuilder.para`.
    """
    st: dict[str, Any] = {}
    if bold:
        st["font_weight"] = "bold"
