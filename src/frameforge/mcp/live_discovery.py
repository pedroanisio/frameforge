...
from frameforge.mcp.execution import _subprocess_env
from frameforge.mcp.paths import _repo_root
...
from frameforge.mcp.util import _utc_now
...
def _source_token(repo_root: Path) -> str:
    ...
    source_root = repo_root / "src" / "frameforge"
    paths = sorted(source_root.rglob("*.py"))
    if not paths:
        raise FileNotFoundError(f"no FrameForge Python sources found under {source_root}")
