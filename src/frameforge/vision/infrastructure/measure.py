...
``import frameforge.vision`` stays dependency-free; the backend is only touched
...
# frameforge.vision.domain.coordinates and are imported above. Only the PIL-bearing
...
# ─────────────────────────────────────────────────────────────
# geometry — pure frame maths moved to frameforge.vision.domain.coordinates
...
# frameforge.vision.domain.coordinates and are imported above.
...
def draw_points_overlay(img, cs: CoordinateSystem,
    ...
    ``frameforge.vision.infrastructure.workspace.render``; ``points`` are already in
