...
shape into the canonical FrameForge document the ``Renderer`` consumes. Relocated
verbatim from ``tooling/render_fixtures.py`` so the rendering bounded context no
longer imports *up* into the tooling scripts — the ``frameforge`` package stays
...
def normalize_doc(doc):
    if not isinstance(doc, dict) or doc.get("dsl") != "FrameForge":
        return doc
    ...
    return {
        ...
        "version": "2.2.0",
        "profile": "deck",
        "title": doc.get("title") or deck.get("title") or "FrameForge presentation deck",
