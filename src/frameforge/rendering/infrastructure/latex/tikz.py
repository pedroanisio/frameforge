"""tikz — FrameForge figure object graph → TikZ, plus LaTeX text/colour helpers.

`FigureTikz.render(object)` returns the *body* of a `tikzpicture` (no wrapper); the
document builder opens the picture with `[x=1pt,y=-1pt]` so FrameForge page-space
...
class FigureTikz:
    ...
    def _path_segments(self, segments):
        return path_segments(segments)

    _arc_to_cubics = staticmethod(arc_to_cubics)
