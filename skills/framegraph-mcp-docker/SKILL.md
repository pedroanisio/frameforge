---
name: framegraph-mcp-docker
description: >-
  Wire any codebase to the dockerized FrameGraph MCP server (SDK authoring,
  rendering, visual QA, raster→vector reconstruction) with full file exchange.
  Use when a project wants to author or render FrameGraph documents via MCP,
  when framegraph tools fail with file-not-found on project paths, when
  rendered output is missing fonts, or when the framegraph tool list looks
...
# FrameGraph over Docker MCP — consumption guide

The `frameforge` image is the canonical FrameGraph runtime: the full SDK and
...
"args": [
  ...
  "-v", "${PWD}:/workspace:ro",
  "-e", "FRAMEGRAPH_MCP_INPUT_ROOTS=/workspace:/work:/app",
  "frameforge"
]
```

- `framegraph-work:/work` — named volume; sessions and written SDK clients
...
- `FRAMEGRAPH_MCP_INPUT_ROOTS` confines file-reading tools to the mounted
...
`framegraph://session/<id>/...` URI (e.g. `page/1.svg`, `document.yaml`,
`diagnostics.json`) returns the content inline. Bulk export without MCP:

```bash
docker run --rm -v framegraph-work:/work -v "$PWD:/out" frameforge \
...
| `input path is outside the allowed FRAMEGRAPH_MCP_INPUT_ROOTS` | File lies outside the mounts — move it into the project or extend the mount |
