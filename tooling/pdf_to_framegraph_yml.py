#!/usr/bin/env python3
"""
pdf_to_framegraph_yml.py
========================

Transpile a PDF into a FrameGraph v2 YAML document.

Goal
----
This is a deterministic, layout-preserving extractor, not a perfect PDF-to-UI
reverse compiler. It maps a PDF page into fixed-layout FrameGraph pages:
...
Codebase reuse
--------------
The output is built to validate against ``models/framegraph.py`` (the project's
source of truth). The document ``version`` is ``framegraph.HEAD_VERSION`` and the
...
the result is structurally self-checked with ``framegraph.Document`` before exit
(disable with ``--no-validate``).

Coordinate system
-----------------
FrameGraph v2 uses top-left origin and +y down. PyMuPDF page coordinates are
...
Usage
-----
    python pdf_to_framegraph_yml.py input.pdf output.framegraph.yml
    python pdf_to_framegraph_yml.py input.pdf output.yml --asset-dir assets
    python pdf_to_framegraph_yml.py input.pdf output.yml --max-pages 3
    python pdf_to_framegraph_yml.py input.pdf output.yml --no-images
    python pdf_to_framegraph_yml.py input.pdf output.yml --no-vectors
    python pdf_to_framegraph_yml.py input.pdf output.yml --text-mode lines
    python pdf_to_framegraph_yml.py input.pdf output.yml --table-mode off
...
import framegraph as fg  # noqa: E402  (resolved via the models/ path inserted above)
...
class TokenStore:
    ...
    ``Style`` projections so the document validates against models/framegraph.py
...
class PDFToFrameGraph:
    ...
    def _assemble(self, pages: list[dict[str, Any]], first_size: list[float] | None) -> dict[str, Any]:
        return {
            ...
            "title": f"FrameGraph extraction — {self.pdf_path.name}",
            "description": "Generated by pdf_to_framegraph_yml.py from a PDF text/layout/vector/image extraction pass.",
            "lang": "und",
            "defs": {
                "tokens": {
                ...
                "ontology": {
                    "node_types": {
                        ...
                        "page": {"meaning": "PDF page converted to a fixed FrameGraph page"},
                    ...
                    "edge_types": {
                        ...
                        "derived_from": {"meaning": "FrameGraph object derived from PDF source element", "directionality": "directed"},
                    },
                },
            },
            "targets": [
    ...
    def _doc_meta(self) -> dict[str, Any]:
        meta: dict[str, Any] = {
            ...
            "generator": "pdf_to_framegraph_yml.py",
            "generator_scope": {
                "ocr": False,
    ...
    # --------------------------------------------------------------------- #
    # Tables (native FrameGraph `type: table` objects)
    # --------------------------------------------------------------------- #
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Transpile PDF pages into a fixed-layout FrameGraph v2 YAML document."
    )
    p.add_argument("input_pdf", type=Path, help="Input PDF")
    p.add_argument("output_yml", type=Path, help="Output FrameGraph YAML")
    p.add_argument(
        "--asset-dir",
        type=Path,
        default=None,
    ...
    p.add_argument(
        "--no-validate",
        action="store_true",
        help="Skip the structural self-check against models/framegraph.py.",
    )
...
def _validate(doc: dict[str, Any]) -> None:
    ...
    try:
        ...
        print("Validation: PASS (conforms to models/framegraph.py)")
...
def main(argv: list[str] | None = None) -> int:
    ...
    converter = PDFToFrameGraph(args.input_pdf, opts)
    framegraph = converter.transpile()
    args.output_yml.parent.mkdir(parents=True, exist_ok=True)
    yaml_dump(framegraph, args.output_yml)

    page_count = len(framegraph.get("pages", []))
    obj_count = sum(
        len(layer.get("objects", []))
        for page in framegraph.get("pages", [])
