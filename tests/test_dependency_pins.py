...
by someone installing from an index. An audit across the eight family
repositories found three kinds of defect that a local test run would never
surface —
...
# The guarded idiom `test_python_version_support.py` enforces: `tomllib` is
# stdlib only on 3.11+, and this module runs under the declared 3.10 floor.
try:
    import tomllib
...
def test_declared_ranges_admit_the_locked_versions():
    """A range that excludes the version actually resolved is a defect a source
    entry hides.

    `uv` honours a git or path source regardless of the declared specifier, so
    a stale cap fails nothing here and fails every install from an index. This
    caught `frameforge-mcp>=1.0,<2` in the engine on the day `frameforge-mcp`
    released 2.0.0 — a cap written hours earlier, already wrong, and invisible
    to `uv lock --check`.
    """
    excluded = []
    for section, name, spec in _family_requirements():
