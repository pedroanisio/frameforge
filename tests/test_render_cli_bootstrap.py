"""framegraph-render bootstrap — works without PYTHONPATH, from the src layout.

Closes issue #35. Two root causes, both src-layout refactor casualties:

- ``framegraph.sdk.model`` hard-imports ``models.framegraph`` and relied on the
  caller exporting ``PYTHONPATH=docs`` — the Makefile compensates, the CLI
  front door did not, so ``uv run framegraph-render … --to svg`` crashed with
  ``ModuleNotFoundError: models``.
- ``framegraph.cli`` derived the repo root as ``HERE/..`` from its pre-refactor
...
def test_sdk_model_resolves_without_docs_on_path(tmp_path):
    ...
    code = ("import sys; sys.path.insert(0, %r); "
            "from framegraph.sdk.model import HEAD_VERSION; print(HEAD_VERSION)" % SRC)
...
def test_tooling_launcher_is_pythonpath_and_cwd_free(tmp_path):
    """`uv run python tooling/framegraph_render.py` is the virtual-project
    ...
    doc.write_text(MINI_DOC, encoding="utf-8")
    out = tmp_path / "out"
    launcher = os.path.join(ROOT, "tooling", "framegraph_render.py")
    proc = subprocess.run(
