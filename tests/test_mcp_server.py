#!/usr/bin/env python3
"""Regression tests for the FrameForge MCP feedback loop."""
...
def test_run_sdk_code_generates_yaml_validates_and_renders_svg(tmp_path):
    ...
    assert any(link["uri"] == "frameforge://session/loop-1/page/1.svg" for link in result["resources"])
...
def test_pages_selector_renders_only_named_pages_with_true_page_numbers(tmp_path):
    ...
    assert [r["uri"] for r in svg_renders] == [
        "frameforge://session/sel/page/2.svg",
        "frameforge://session/sel/page/3.svg",
    ]
...
def test_rasterization_respects_page_cap_and_uses_true_page_filenames(tmp_path, monkeypatch):
    ...
    assert png_renders[0]["uri"] == "frameforge://session/cap2/page/2.png"
...
def test_read_session_resource_returns_yaml_svg_and_diagnostics(tmp_path):
    ...
    document = read_session_resource("frameforge://session/resources/document.yaml", session_root=tmp_path)
    page = read_session_resource("frameforge://session/resources/page/1.svg", session_root=tmp_path)
    diagnostics = read_session_resource(
        "frameforge://session/resources/diagnostics.json",
        session_root=tmp_path,
    )
...
def test_create_server_registers_feedback_loop_tools_and_resources(tmp_path):
    ...
    assert "frameforge://session/{session_id}/document.yaml" in server.resources
    assert "frameforge://session/{session_id}/page/{page_number}.svg" in server.resources
    assert "frameforge://session/{session_id}/page/{page_number}.png" in server.resources
    assert "frameforge://session/{session_id}/diagnostics.json" in server.resources
    assert "frameforge://session/{session_id}/document.pdf" in server.resources


def test_create_server_registers_authoring_guide_prompt(tmp_path):
    server = create_server(session_root=tmp_path, fastmcp_cls=FakeFastMCP)

    assert "frameforge_guide" in server.prompts
    guide = server.prompts["frameforge_guide"]()
...
def test_new_generated_yaml_uses_content_hash_not_mtime(tmp_path):
    ...
    fixture.write_text("dsl: FrameForge\n", encoding="utf-8")
    before = server._frameforge_yaml_snapshot(tmp_path)
...
def test_run_sdk_code_tool_bounds_streams_but_diagnostics_keep_full(tmp_path):
    ...
    diagnostics = read_session_resource(
        "frameforge://session/chatty/diagnostics.json", session_root=tmp_path
    )
...
def test_session_page_png_resource_returns_png_bytes(tmp_path):
    server = create_server(session_root=tmp_path, fastmcp_cls=FakeFastMCP)
    uri = "frameforge://session/{session_id}/page/{page_number}.png"
...
def test_render_refuses_oversized_document_before_rendering(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("FRAMEFORGE_MCP_RENDER_MAX_OBJECTS", "1")
...
def test_mcp_guide_names_headline_sdk_capabilities():
    ...
    The capability list inside FRAMEFORGE_GUIDE is hand-maintained prose and has
    ...
    missing_guide = [name for name in headline if name not in FRAMEFORGE_GUIDE]
    assert missing_guide == [], (
        f"FRAMEFORGE_GUIDE omits headline SDK capabilities {missing_guide} — update the "
        "guide in frameforge/mcp/server.py when the SDK surface grows"
    )
...
def test_detect_regions_tool_returns_regions_and_overlay(tmp_path):
    ...
    assert result["renders"][0]["uri"] == "frameforge://session/regions/page/1.png"
...
def test_run_sdk_code_sign_embeds_provenance_metatag(tmp_path):
    ...
    assert 'xmlns="https://frameforge.dev/ns/provenance"' in meta
