...
def test_out_of_profile_superset_is_warn():
    ...
    for t in ("graph", "use", "component", "bar_chart"):
        assert t in blob, f"expected out-of-profile WARN to mention {t!r}"
    # Connector and the absorbed UML family are typed at HEAD: they moved from
    # the out-of-profile superset into the VisualObject union.
    assert "'connector'" not in blob, "typed connector must no longer be out-of-profile"
    assert "uml.actor" not in blob, "typed UML objects must no longer be out-of-profile"
