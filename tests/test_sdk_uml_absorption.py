...
# The fourteen `uml-*.fg.yaml` fixtures are GENERATED by the cookbook's
# `uml_composers.py --update-fixtures`, and that generator left with the corpus
# (it is now in `frameforge-example`). The generator-vs-fixture equality check
# went with it — it asserts a property of the generator, and running it here
# would give this repo a test-time edge back into the cookbook.
#
# What stays is everything the fixtures can answer on their own: the diagram set
# is complete, each fixture really is a single page named for its file, and the
# pinned render hashes still hold. A regression in the composers reaches these
# fixtures through a regenerate, and this notices.
UML_DIAGRAMS = [
    "class",
    "package",
    "use-case",
...
def test_uml_fixture_set_is_complete_and_each_is_a_single_named_page():
    fixture_dir = Path(__file__).parents[1] / "tests" / "fixtures"
    on_disk = sorted(p.stem.removeprefix("uml-").removesuffix(".fg")
                     for p in fixture_dir.glob("uml-*.fg.yaml"))
    assert on_disk == sorted(UML_DIAGRAMS), (
        "the generated UML fixture set drifted — regenerate with "
        "`uml_composers.py --update-fixtures` in frameforge-example")

    for diagram in UML_DIAGRAMS:
        single = parse((fixture_dir / f"uml-{diagram}.fg.yaml").read_text(encoding="utf-8"),
                       forgiving=False)
        assert [candidate.id for candidate in single.pages] == [diagram]


def test_uml_golden_lock_pins_the_diagrams_it_claims_to():
    """The lock stays a real pin over fixtures this repo owns.

    The hash *comparison* moved with the generator: the locked values were taken
    over pages inside `build()`'s fourteen-page document, and re-deriving them
    from the split single-page fixtures is a different computation, not the same
    assertion moved. `frameforge-example` runs the original check against these
    files. What is verifiable here is that the lock has not quietly emptied and
    still names fixtures that exist.
    """
    lock_path = Path(__file__).parents[1] / "tests" / "golden" / "uml-composers.lock.json"
    fixture_dir = Path(__file__).parents[1] / "tests" / "fixtures"
    locked = json.loads(lock_path.read_text(encoding="utf-8"))

    assert locked, "uml-composers.lock.json is empty — the pin became vacuous"
    missing = [d for d in locked if not (fixture_dir / f"uml-{d}.fg.yaml").is_file()]
    assert not missing, f"lock pins diagram(s) with no fixture: {missing}"
    assert set(locked) <= set(UML_DIAGRAMS), (
        f"lock names diagrams outside the composer set: {sorted(set(locked) - set(UML_DIAGRAMS))}")
    assert all(len(h) == 64 for h in locked.values()), "lock holds a non-sha256 value"
